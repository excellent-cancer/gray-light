...
import MarkdownItHeadline from './markdown-it-headline'
...
export default {
  ...
  watch: {
    request(value, old) {
      if (old != null) {
        let scroll = this.htmlCache[old.id].scroll
        scroll.disable()
      }

      if (value == null) {
        this.$refs['markdown-it-vue-container'].innerHTML = ""
      } else if (this.htmlCache[value.id]) {
        this.$refs['markdown-it-vue-container'].innerHTML = this.htmlCache[value.id].html
        this.htmlCache[value.id].scroll.enable()
      } else {
        this.$nextTick(() => {
          const cache = {
            html: null,
            scroll: this.$scrollToFactory.newGroup({
              active: scroll => this.$emit("scroll", value, scroll),
              offset: -30,
              over: this.$vuetify.application.top
            }),
            headlines: []
          }
          ...
          this.htmlCache[value.id] = cache
          this.$emit("replaced", value, cache.headlines)
        })
      }
    }
  },
  data() {
    ...
    let md = new MarkdownIt(optMarkdownIt)
      ...
      .use(MarkdownItHeadline, { level: 4, hash: this.hash })
